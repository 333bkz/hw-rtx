...
public class Manager extends Emitter {
    ...
    private final Set<Socket> connecting = new HashSet<>();
    ...
    public Manager open(final OpenCallback fn) {
        EventThread.exec(() -> {
            ...
            if (Manager.this._timeout >= 0) {
                ...
                Manager.this.subs.add(timer::cancel);
            }
    ...
    private void onopen() {
        ...
        this.subs.add(On.on(socket, Engine.EVENT_DATA, objects -> {
        ...
        this.subs.add(On.on(socket, Engine.EVENT_PING, objects -> Manager.this.onping()));
        this.subs.add(On.on(socket, Engine.EVENT_PONG, objects -> Manager.this.onpong()));
        this.subs.add(On.on(socket, Engine.EVENT_ERROR, objects -> Manager.this.onerror((Exception) objects[0])));
        this.subs.add(On.on(socket, Engine.EVENT_CLOSE, objects -> Manager.this.onclose((String) objects[0])));
        this.decoder.onDecoded(Manager.this::ondecoded);
    }
    ...
    public Socket socket(final String nsp, Options opts) {
        ...
        if (socket == null) {
            ...
            if (_socket != null) {
                ...
                socket.on(Socket.EVENT_CONNECTING, args -> self.connecting.add(s));
                socket.on(Socket.EVENT_CONNECT, objects -> s.id = self.generateId(nsp));
            }
        }
        return socket;
    }
